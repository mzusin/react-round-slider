...
describe('Max Pointer', () => {
    ...
    it('returns the maximum values of rx and ry from the pointers array', () => {
        const pointers: IStatePointer[] = [
            { pointerRadii: [5, 10], percent: 0.5, id: '1', index: 0 },
            { pointerRadii: [7, 8], percent: 0.3, id: '2', index: 1 },
            { pointerRadii: [3, 6], percent: 0.7, id: '3', index: 2 },
        ];
    ...
    it('returns [0, 0] when all rx and ry values are negative', () => {
        const pointers: IStatePointer[] = [
            { pointerRadii: [-5, -10], percent: 0.2, id: '1', index: 0 },
            { pointerRadii: [-7, -8], percent: 0.4, id: '2', index: 1 },
            { pointerRadii: [-3, -6], percent: 0.6, id: '3', index: 2 },
        ];
        const result: Vector2 = getMaxPointer(pointers);
        expect(result).toEqual([0, 0]);
    });
    ...
    it('returns the maximum values when rx and ry are 0', () => {
        const pointers: IStatePointer[] = [
            { pointerRadii: [0, 0], percent: 0.2, id: '1', index: 0 },
            { pointerRadii: [2, 4], percent: 0.4, id: '2', index: 1 },
            { pointerRadii: [0, 6], percent: 0.6, id: '3', index: 2 },
        ];
        const result: Vector2 = getMaxPointer(pointers);
